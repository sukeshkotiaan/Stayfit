...
function App() {
  ...
  const screen = (location.pathname === "/" || location.pathname === "") ? "landing" : location.pathname.substring(1);
  const setScreen = (s) => navigate(`/${s}`);
  ...
  if (screen === "landing") {
    return <LandingPage isDark={isDark} C={C} S={S} navigate={navigate} />;
  }
  ...
  if (screen === "onboard") {
    return <OnboardScreen 
      currentUser={currentUser} onboardStep={onboardStep} setOnboardStep={setOnboardStep}
      onboard={onboard} setOnboard={setOnboard} setScreen={setScreen}
      COLORS={C} FONTS={FONTS} S={S} isDark={isDark} notify={notify} scheduleSlots={scheduleSlots}
      sbUpdateUser={sbUpdateUser} sbAddLog={sbAddLog} loadLogs={loadLogs} 
      setCurrentUser={setCurrentUser} saveSession={saveSession}
    />;
  }
